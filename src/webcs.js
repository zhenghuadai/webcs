...
export class CSKernel {
   ...
   *
   * @param {} arg -   a list of shader arguments, such as run(arg0, arg1, ...,
   *     argn)
   * @param {unfold_of_vec3} groups_xyz - optional, the size of work group,
   *     such as run(arg0, arg1, ..., argn, groups_x, groups_y, groups_z)
   * @param {{}} uniform    - optional, uniform , such as run(arg0, arg1, ...,
   *     argn, groups_x, groups_y, groups_z, {uniform_name:uniform[0,1,2,3]})
   * @example
   * run(arg0, arg1, ..., argn)
   * run(arg0, arg1, ..., argn, groups_x, groups_y, groups_z)
   * run(arg0, arg1, ..., argn, groups_x, groups_y,
   * groups_z, {uniform_name:uniform[0,1,2,3]}) run(arg0, arg1, ...,
   * argn, {uniform_name:uniform[0,1,2,3]})
   */
  async run(...args) {
  ...
  setUniform(name, ...values) {
    ...
    if (this.uniformVids[slot] == null) {
      this.uniformVids[slot] = device.createBuffer(
          { mappedAtCreation: true, size: bufferSizeInBytes, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
  ...
  async __updateArg(i, arg) {
    ...
    if (isBuffer(argType)) {
      ...
      if (arg == null) {
        ...
        this.vids[i] = arg;
        // arg.unmap(); // Typically shouldn't unmap if passed in, unless
        // we know it's mapped
    ...
    for (let pi = 0; pi < params.length; pi++) {
      ...
      if (param_type.dim === 1) { // buffer
        ...
        layout_str += ` struct struct_${param_name}{ data: ${final_type}} ;\n@group(0) @binding(${
            pi}) var<storage, read_write> ${param_name} : struct_${param_name};\n`;
    ...
    const getHostAccessArrary = (dstarray, arrayBuffer) => {
      if (dstarray == undefined) {
        ...
        const typemap    = new Map([
          ['int8', Int8Array], ['uint8', Uint8Array], ['uint8clamped', Uint8ClampedArray], ['int16', Int16Array],
          ['uint16', Uint16Array], ['int32', Int32Array], ['uint32', Uint32Array], ['float32', Float32Array],
          ['float', Float32Array], ['float64', Float64Array], ['bigint', BigInt64Array], ['bigint', BigUint64Array]
           ]);
